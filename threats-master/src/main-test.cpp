...
//    cout << "Creation of the test vectors" << endl;
    static const uint32_t nslots = 128; // ! modif
...
// ------------------------------------------------------
    ...
    for (int i = 0; i < loop; ++i) {
        ...
        if ( ((sample_client_false[r1] == 0) && (tmp_result == 0)) || ( tmp_result == (256-sample_client_false[r1])) )
        {
            cnt++;
            printf("The result is %ld\n", tmp_result);
        }
    ...
    for (int i = 0; i < loop; ++i) {
        ...
        if ( ((sample_client_false[r1] < 128) && (tmp_result == sample_client_false[r1])) || ((sample_client_false[r1] >= 128) && (tmp_result == -(sample_client_false[r1]-256))))
        {
            cnt++;
        }
        else
        {
            if (sample_client_false[r1] < 128)
                printf("The result is %ld and should be %d\n", tmp_result, sample_client_false[r1]);
            else
                printf("The result is %ld and should be %d\n", tmp_result, sample_client_false[r1]-128);
        }
    ...
    */
    ...
    cout << "Test of the Manhattan distance and the Euclidean distance between two vectors of n-bit numbers" << endl;
    cout << "Vector v1" << endl;
    printVector(template_client, nslots);
    ...
    cout << "The result shall be as follows:" << endl <<  "ManhattanDistance(v1,v2)= " << mh << "and" << endl << "EuclideanDistance(v1,v2)= " << eh << endl;
    for (int i = 0; i < max_bitsize; ++i) {
